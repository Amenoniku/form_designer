...
# Описание.
Все данные формы хранятся в `content/resources/data/testForm.json`. Можно редактировать json как угодно но есть несколько условий

#### Рекомендации к JSON

Все поля содержатся в объекте fields и у них есть несколько доп. параметров

- required — обязательное поле
- addField — добавить поля
- checkAll — выбрать все чекбоксы для checkbox

Для активации укажите `true`
Напимер:
```
"required": true
```

Единственное обязателеный параметр это dir — название директивы отвечающая за соответствующие поле. Можно конечно было обойтись без него но по мойму так удобней.
